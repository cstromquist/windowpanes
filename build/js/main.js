...
var Panes = (function () {
	...
	_createClass(Panes, [{
		...
		key: 'removeAccordionEvents',
		value: function removeAccordionEvents() {
			this.$panes.find('.pane:not(.no-toggle)').removeClass('accordion-toggle').addClass('pane-toggle');
		}
	}, {
		key: 'addAccordionEvents',
		value: function addAccordionEvents() {
   ...
   */
		...
		key: 'toggleVerticalPane',
		value: function toggleVerticalPane(index, callback) {
			var _this = this;
			...
			TweenLite.to(".video-wrapper", 0.3, { opacity: 0, onComplete: function onComplete() {
					$(".video-wrapper").html("");
			...
			if ($pane.length) {
				...
				if ($pane.hasClass('open')) {
					...
					TweenLite.to($pane, 1, { left: startPoint, ease: Quad.easeOut, onComplete: function onComplete() {
		...
		key: 'slidePanesRight',
		value: function slidePanesRight() {
   ...
   */
		...
		key: 'toggleSquarePane',
		value: function toggleSquarePane(index) {
			var _this = this;
			var $pane = this.$panes.find(".pane[data-index=" + index + "]");
			...
			if ($paneContent.length && $pane.length) {
				...
				if (this.currentPaneIndex == index) {
					...
					if (this.$currentPane.length) {
						...
						TweenLite.to($previousPaneContent, 1, { left: -this.paneContentWidth, ease: Quad.easeOut, onComplete: function onComplete() {
